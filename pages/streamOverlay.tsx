    ...
    <>
      <Head>
        ...
        <meta name="og:description" property="og:description"
              content="Admin screen where the views of the contestants be controlled" />
      </Head>
      <div className={"flex flex-col justify-start h-full bg-transparent"}>
        <div className={"h-full"}>
          {state.currentGameData && state.currentGameData.show_question &&
          <div className={"flex flex-col justify-start h-full"}>
            <div className={"h-huge w-1/2 p-5 bg-dsm-bg-dark rounded"}>
              ...
              <div className={"flex flex-col justify-start self-start"}>
                <div className={"relative oval bg-dsm-bg w-16 h-16 self-center"}>
                  <h1
                    className={"absolute bottom-1/2 left-1/2 transform translate-y-1/2 -translate-x-1/2 text-white text-2xl"}>{state.currentGameData.round1}</h1>
                </div>
                <CorrectAnswers gameData={state.currentGameData} socket={state.socket} isAdmin={false} />
              ...
              <div className={"mt-10"}>
                <Round3 answers={state.currentGameData.current_question.answers}
                        correct={state.currentGameData.current_answered} />
              </div>
              }
            </div>
          </div>
